      ...
      <?php
       ...
       //felhazsnalo adatainek kiolvasasa     
		...
		// szamla szam generalas és beallitott megjegyzesek
			...
			$select="SELECT * FROM szla_beallitasok where user_id=$id";
		...
		//ha létezik a megjegyzes tipus akkor hozzáadunk egy sortörést a végéhez, hogy össze tudjuk őket fűzni
			...
			//ha nincs beállítva előtag akkor automatikusan az év lesz
				if ($szla_elotag=="") 
					{
					$szla_elotag = date("Y");
					$insert="INSERT INTO szla_beallitasok (user_id, szla_elotag) VALUES ('$id', '$szla_elotag')";
					$result = $con->query($insert) or die(mysql_error());;					
					}
			//szam
			$select="SELECT MAX(szla_szam) AS max_szla_szam FROM szla_fej where user_id='$id' and szla_elotag='$szla_elotag'";
			$result = mysqli_query( $con, $select );
			$row = mysqli_fetch_assoc($result);
		...
		//	számla tételek
			...
			for ($i = 0; $i <= $tombsor; $i++) {
					...
					if ($szla_egysegar_tipus=="brutto")
						...
						$tetel_netto_egysegar = round($tetel_netto_egysegar / ((100 + $tetel_afakulcs)/100), 0);
						} 
						else
						{
						$tetel_netto_egysegar = ($_POST['szamla_tetel_egysegar'][$i]);	
					...
					//ha mező nem üres akkor közveteitte szolgalatatsarol bezsélünk és az adatbazisban true-nek irjuk mert az NAV xml-be igy kell szerepeltetni.
					if ($_POST['szamla_tetel_kozv_szolg'][$i]!="")
						{
						$tetel_kozv_szolg='true';
						} 

			$sql = "INSERT INTO `szla_tetel` (`table_line_id` , `user_id`, `tetel_megnevezes`, `tetel_kozv_szolg`, `tetel_mennyiseg`, `tetel_mennyisegi_egyseg`, `tetel_netto_egysegar`, `tetel_afakulcs`, `tetel_afa_nev`, `tetel_netto_ertek`, `tetel_afaertek`, `tetel_brutto_ertek`, `szla_elotag`, `szla_szam`, `szla_sorszam`, `szla_datum`) VALUES (NULL, '$user_id', '$tetel_megnevezes', '$tetel_kozv_szolg', '$tetel_mennyiseg', '$tetel_mee', '$tetel_netto_egysegar', '$tetel_afakulcs', '$tetel_afa_nev', '$tetel_netto_ertek', '$tetel_afaertek', '$tetel_brutto_ertek', '$szla_elotag', '$szla_szam', '$szla_sorszam', '$szla_datum');";
